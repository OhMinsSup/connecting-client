...
const PORTAL_CLASSNAME = 'connecting-portal'
const PORTAL_SELECTOR = '.connecting-portal'

const Container: React.FC<{ zIndex: number }> = (props) => (
  <div
    className="app-portal-zIndex"
    style={{
      position: 'absolute',
      zIndex: props.zIndex,
      top: 0,
...
const DefaultPortal: React.FC<{ appendToParentPortal?: boolean }> = (props) => {
  ...
  return portal.current ? createPortal(<PortalProvider value={portal.current}>{_children}</PortalProvider>, portal.current) : <span ref={tempNode} />
}
...
const ContainerPortal: React.FC<ContainerPortalProps> = (props) => {
  ...
  if (host && portal) {
    return createPortal(<PortalProvider value={appendToParentPortal ? portal : null}>{children}</PortalProvider>, portal)
  }
